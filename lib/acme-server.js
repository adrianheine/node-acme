...
class Certificate {
  ...
  issue() {
    this.body = this.server.CA.issueCertificate(this.csr, this.notBefore, this.notAfter);
  }
...
class ACMEServer {
  ...
  constructor(options) {
    ...
    this.transport = new TransportServer(this.acmeVersion);
    ...
    this.app.post(basePath + DIRECTORY_TEMPLATE['new-reg'], (req, res) => this.newReg(req, res));
    this.app.post(basePath + '/reg/:id', (req, res) => this.updateReg(req, res));
    this.app.post(basePath + DIRECTORY_TEMPLATE['new-app'], (req, res) => this.newApp(req, res));
    this.app.post(basePath + DIRECTORY_TEMPLATE['new-cert'], (req, res) => this.newCert(req, res));
    this.app.post(basePath + '/authz/:id/:index', (req, res) => this.updateAuthz(req, res));
    this.app.post(basePath + DIRECTORY_TEMPLATE['new-authz'], (req, res) => this.newAuthz(req, res));
    // TODO others
  }
  ...
  newAuthz(req, res) {
    ...
    let name = req.payload.identifier.value;

    let authz = new Authorization(this, thumbprint, name);
    this.db.put(authz);
    res.status(201);
    res.send({challenges: [{type: 'http-01', token: 'token', uri: authz.url + '/0' }]});
  }
  ...
  newCert(req, res) {
    ...
    // Create a stub application
    let cert = new Certificate(this, thumbprint);
    ...
    try {
      ...
      if (req.payload.notBefore) {
        ...
        cert.notBefore = req.payload.notBefore;
      } else {
      ...
      if (req.payload.notAfter) {
        ...
        cert.notAfter = req.payload.notAfter;
      } else {
        cert.notAfter = new Date();
        cert.notAfter.setFullYear((new Date()).getFullYear() + 1);
      }
    } catch (e) {
      res.status(400);
  ...
  updateAuthz(req, res) {
    ...
    authz.challengeObj[index].update(res.payload)
      ...
      .then(() => {
        res.status(this.acmeVersion === 'le' ? 202 : 200);
        res.send(authz.challenges[index]);
        res.end();
      });
  }
